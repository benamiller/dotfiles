...
require("lazy").setup({
...

	{
		...
		keys = {
			...
			{ "<leader>gS", ":Git status<CR>", desc = "Git status (full)" },
			...
			require("telescope").setup({
				...
				defaults = {
					file_ignore_patterns = {
						"node_modules/.*", -- Ignores everything inside node_modules
					},
					hidden = true,
				},
				-- pickers = {}
		...
		config = function()
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					-- Get error float
					map("<leader>e", vim.diagnostic.open_float, "Show [E]rror details")
	...
	{ -- Autoformat
		...
		opts = {
			...
			format_on_save = function(bufnr)
				-- Simplify: always format Elm, skip only c/cpp
				if vim.bo[bufnr].filetype == "c" or vim.bo[bufnr].filetype == "cpp" then
					return nil -- Skip formatting for C/CPP
				end
			...
			formatters = {
				["elm-format"] = {
					command = vim.fn.stdpath("data") .. "/mason/bin/elm-format",
	...
	{ -- Highlight, edit, and navigate code
		...
		opts = {
			...
			indent = { enable = false, disable = { "ruby", "elm" } },
		},
...
vim.api.nvim_create_autocmd("FileType", {
	pattern = "elm",
	callback = function()
		vim.bo.indentexpr = "" -- Disable any indentexpr
		vim.bo.autoindent = true -- Match previous line
		vim.bo.shiftwidth = 2 -- 2-space indents
		vim.bo.tabstop = 2
